...
class ChatRoomNotifier extends _$ChatRoomNotifier {
  ...
  @override
  Future<List<MessageData>> build({required String roomCode}) async {
    currentUsername = ref.read(authViewModelProvider.notifier).getCurrentUser()?.username;
        ...
        if (action == "add") {
          updatedReactions.add(
            MessageReact(
              emoji: emoji,
              messageId: message.id,
              senderId: currentUsername!, // Using username as ID for simplicity
  ...
  void deleteMessage({required String msgId}) async {
    try {
